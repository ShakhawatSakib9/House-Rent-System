...
• 4-Tier relational property hierarchy: Property → Building → Floor → Flat / Unit
• Automated lease lifecycle state synchronization (Vacant ↔ Occupied flat transitions)
• Batch monthly rent and utility billing engine (Water, Gas, Electricity, Service Charge)
...
• Production deployment readiness with containerized Dockerfile and Vercel serverless configuration
```
...
5. [4-Tier Property Hierarchy Modeling](#-5-4-tier-property-hierarchy-modeling)
...
graph TB
    ...
    subgraph TenantContext["Tenant Landlord Workspace (/user)"]
        ...
        F["4-Tier Property Manager: Property → Building → Floor → Flat"]
...
| **Plan Limit Gating** | Trait-Based Interceptor (`ChecksPlanLimits`) | Encapsulates plan quota checks (max buildings, max units, max users) cleanly within model creation hooks without cluttering controller code. |
...
| **4-Tier Property Hierarchy** | ✅ **Implemented** | `Property` → `Building` → `Floor` → `Flat/Unit` with relational cascading |
...
| **Container & Cloud Deployment** | ✅ **Implemented** | Production-ready `Dockerfile` and Vercel serverless configuration (`vercel.json`) |

---

## 🏘️ 5. 4-Tier Property Hierarchy Modeling
...
- **Authorization Guarded Streaming:** Every file download passes through a dedicated controller action that validates the requesting user's tenant project scope before streaming the binary payload.
...
- **Pre-Creation Interception:** When a landlord attempts to create a new building or flat, the trait queries the active subscription quota. If the cap is reached, the request is rejected with a structured upgrade prompt.
...
**Solution:** Engineered the `ChecksPlanLimits` trait, intercepting model creation requests at the service layer and validating active subscription quotas against current entity counts.
...
- **Batch Billing Generation:** Validated that monthly recurring invoice batches compute exact rent and variable utility sums across active leases.
